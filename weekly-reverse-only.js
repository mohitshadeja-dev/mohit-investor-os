...
<div class=note>Primary setup is virtual only. Its P&L is ignored. A trade is taken only after that virtual primary fails at the Gann SL on a completed 5-minute close. Reverse target = 100 points. Reverse SL = opposite Gann boundary. Position is carried positionally until target or SL. No repeated reversal.</div>
...
function body(){return {symbol:$('wkSym').value.trim()||'NIFTY 50',from_date:$('wkFrom').value,to_date:$('wkTo').value,wma_factor:+$('wkWma').value,gann_step:+$('wkStep').value,target_points:100,gap_near_target_points:+$('wkGap').value,same_bar_policy:'stop_first'}}
